...
snowcover_data_fixed <- snowcover_data %>% 
  mutate(Date2 = as_date(Date)) #72 rows it failed to conver to a date 
```
...
snowcover_data_fixed %>% 
  filter(is.na(Date2)) #72 dates with the weird 8&9 format
```
...
#only counting more than 10%, cumulative snow days, mean snow cover
snowcover_data_fixed$Month = month(snowcover_data_fixed$Date2)


snowcover_data_fixed <- snowcover_data_fixed %>% 
  mutate(snow_days = ifelse(Snow_cover > 10, 1, 0)) %>% 
  group_by(Site, Year,Month) %>% 
  summarise(avg_snow_cover = mean(Snow_cover, na.rm = TRUE), cum_snow_days = sum(snow_days, na.rm = TRUE))
...
nests_snow_month <- nests_month %>% 
  inner_join(snowcover_data_fixed, join_by(Site, month==Month, Year)) #only want to keep what is common between the two
```
...
nests_snow_year <- nests_snow_month %>% 
  ...
  summarise(cum_snow_days = sum(cum_snow_days, na.rm = TRUE), avg_snow_cover = mean(avg_snow_cover, na.rm = TRUE), n_nest = sum(n_nest, na.rm = TRUE))
```
